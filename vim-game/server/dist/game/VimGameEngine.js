...
export class VimGameEngine {
     ...
     */
    processNormalModeCommand(command) {
     ...
     */
    processInsertModeCommand(command) {
        if (command === '<Esc>') {
     ...
     */
    processVisualModeCommand(_command) {
    ...
    isEfficient(command) {
        ...
        // Redundant movement
        if (recent.includes('h') && recent.includes('l'))
            return false;
        if (recent.includes('j') && recent.includes('k'))
            return false;
        // JetBrains commands are always efficient!
        if (this.getCommandType(command) === 'jetbrains')
            return true;
        // Efficient patterns
        if (['w', 'b', '0', '$', 'gg', 'G'].includes(command))
            return true;
        return true;
    }
    /**
     ...
     */
    generateFeedback(command, _result) {
        const feedbackMap = {
...
//# sourceMappingURL=VimGameEngine.js.map